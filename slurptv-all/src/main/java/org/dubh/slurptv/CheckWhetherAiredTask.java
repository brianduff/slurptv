...
class CheckWhetherAiredTask extends AbstractTask {
  ...
  public EpisodeState perform(Show show, EpisodeState previousState)
      throws TaskFailedException, InterruptedException {
		if (!show.hasTvdbId()) {
			// No tv database id, so we can't check. Just transition to the next state.
		...
		try {
			...
			if (details == null) {
				...
				log.log(episodeLog.info(show, previousState.getEpisode(), "Not found in tvdb. Assuming not yet aired."));
			...
			if (!details.hasAirDateMillis()) {
				...
				log.log(episodeLog.info(show, previousState.getEpisode(), "No airdate yet in tvdb. Assuming not yet aired."));
				return previousState;
			}
	...
	private EpisodeState successState(EpisodeState previousState) {
	  return EpisodeState.newBuilder(previousState)
	  		.setLastCompletedStep(Step.CHECKING_WHETHER_AIRED)
	  		.build();
  }
