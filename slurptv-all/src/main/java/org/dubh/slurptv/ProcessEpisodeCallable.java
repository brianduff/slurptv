...
public class ProcessEpisodeCallable implements Callable<Void> {
  ...
  private void checkForScrewedUpState(EpisodeState episodeState, EpisodeState newState) {
  ...
  private EpisodeState runNextStep(Show show, Episode episode, EpisodeState oldState) {
    ...
    try {
      ...
      return EpisodeState.newBuilder(newState).clearFailedReason().clearFailedStep()
      ...
      return EpisodeState.newBuilder(oldState).setFailedStep(nextStep)
          .setFailedReason(t.getMessage()).setRetryCount(oldState.getRetryCount() + 1)
          .setLastAttempt(new DateTime().getMillis()).build();
    }
