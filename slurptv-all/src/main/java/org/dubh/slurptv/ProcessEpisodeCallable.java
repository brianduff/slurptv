...
public class ProcessEpisodeCallable implements Callable<Void> {
  private static final Logger log = Logger
      .getLogger(ProcessEpisodeCallable.class.getName());
  ...
  ProcessEpisodeCallable(StateManager stateManager, EpisodeLog episodeLog,
      ImmutableMap<Step, ? extends AbstractTask> tasks,
      Configuration configuration, Show show, Episode episode) {
  ...
  public Void call() throws IOException {
    ...
    if (episodeState.getRetryCount() > configuration.getMaxRetries()) {
      log.log(episodeLog.info(show, episode,
          "Skipping because retry count was exceeded"));
      return null;
    }
    while (episodeState.getLastCompletedStep() != Step.DONE) {
      ...
      if (oldState.getLastCompletedStep() == episodeState
          .getLastCompletedStep()) {
        log.log(episodeLog.info(show, episode,
            "Didn't make progress. Giving up for now."));
        break;
      }
    }
    return null;
  ...
  private void checkForScrewedUpState(EpisodeState episodeState,
    ...
    if (!newState.getEpisode().equals(episodeState.getEpisode())) {
      throw new IllegalStateException("Step "
          + findNextStep(episodeState.getLastCompletedStep())
          + " screwed up the state by wiping episode.");
    }
  }

  private EpisodeState runNextStep(Show show, Episode episode,
      EpisodeState oldState) {
    ...
    try {
      ...
      return EpisodeState.newBuilder(newState).clearFailedReason()
          .clearFailedStep().setLastAttempt(new DateTime().getMillis()).build();
    } catch (Throwable t) {
      log.log(episodeLog.severe(show, episode, "Failed step due to exception "
          + nextStep, t));
      return EpisodeState.newBuilder(oldState).setFailedStep(nextStep)
          .setFailedReason(t.getMessage())
          .setRetryCount(oldState.getRetryCount() + 1)
          .setLastAttempt(new DateTime().getMillis()).build();
    }
