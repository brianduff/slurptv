...
public class FindDownloadFileTask extends AbstractTask {
  ...
  @Inject
  public FindDownloadFileTask(Configuration configuration, EasynewsService easynews) {
  ...
  public EpisodeState perform(Show show, EpisodeState previousState) throws TaskFailedException {
    ...
    List<String> extensions = configuration.getFileConfiguration().getFileTypeList();
    if (show.hasFileConfiguration() && show.getFileConfiguration().getFileTypeCount() > 0) {
      extensions = show.getFileConfiguration().getFileTypeList();
    }
    String preferredExtension = configuration.getFileConfiguration().getPreferredType();
    if (show.hasFileConfiguration()) {
      preferredExtension = show.getFileConfiguration().getPreferredType();
    }

    ImmutableSet<ResultFilter> filters = ImmutableSet.of(new AtLeastSizeFilter(minSize),
        new NoBiggerThanFilter(maxSize), new FilenameExtensionFilter(extensions));
    ImmutableSet<ResultRanker> rankers = ImmutableSet.of(new PreferExtensionRanker(
        preferredExtension), new PreferEnglishRanker());

    Query query = new Query().setShowTitle(show.getName()).setEpisode(previousState.getEpisode());

    try {
      List<Result> results = easynews.findFiles(query, filters, rankers);
      if (results.size() == 0) {
        throw new TaskFailedException("No results found for query " + query.getSearch());
      }
      return EpisodeState.newBuilder(previousState).setLastCompletedStep(Step.SEARCHING_EASYNEWS)
          .setUrl(results.get(0).getUrl()).build();
    } catch (IOException e) {
      throw new TaskFailedException("Easynews query " + query.getSearch() + " failed.", e);
    }
  }
}
