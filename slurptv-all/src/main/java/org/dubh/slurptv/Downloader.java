...
class Downloader {
  private static final Logger log = Logger.getLogger(DownloadVideoTask.class
      .getName());
  ...
  public void download(Credentials credentials, String url, File destinationFile)
    ...
    executor.execute(
        new String[] { "curl", "--basic", "--user",
            credentials.getUsername() + ":" + credentials.getPassword(), url,
            "--output", destinationFile.getPath() }, destinationFile.getName());
  }

  public void download(String url, File destinationFile) throws IOException,
      InterruptedException {
    destinationFile.getParentFile().mkdirs();
    executor.execute(
        new String[] { "curl", url, "--output", destinationFile.getPath() },
